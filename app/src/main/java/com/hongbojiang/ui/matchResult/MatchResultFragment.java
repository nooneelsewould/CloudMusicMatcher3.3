...
import com.hongbojiang.PlaylistAPIHelper;
...
public class MatchResultFragment extends Fragment {
    ...
    private static class MatchTaskAsync extends AsyncTask<Void, Void, List<Song>> {
        ...
        protected List<Song> doInBackground(Void... voids) {
            String songBaseURL = "https://music.163.com/song?id=";
            List<com.hongbojiang.proxy.Song> _songs = new ArrayList<>();
            PlaylistAPIHelper playlistAPIHelper = new PlaylistAPIHelper(id_1);
            try {
                _songs = playlistAPIHelper.getSongs();
            } catch (IOException e) {
                e.printStackTrace();
            }
            if (!_songs.isEmpty()) {
                for (com.hongbojiang.proxy.Song item : _songs) {
                    Song song = new Song(item.getName(), songBaseURL + item.getID().toString(), item.getID().toString());
                    set.add(song);
                }
            }
            ...
            playlistAPIHelper = new PlaylistAPIHelper(id_2);
            try {
                _songs = playlistAPIHelper.getSongs();
            } catch (IOException e) {
                e.printStackTrace();
